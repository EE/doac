...
from doac.exceptions.invalid_request import AuthorizationCodeNotValid
...
class TestTokenErrors(TokenTestCase):
    ...
    def test_code_expiration(self):
        self.authorization_token.expires_at = datetime.datetime.now() - datetime.timedelta(days=1)
        self.authorization_token.save()

        response = self.client.post(reverse("oauth2_token"), {"grant_type": "authorization_code", "client_id": self.oauth_client.id, "client_secret": self.client_secret, "code": self.authorization_token.token})
        self.assertExceptionJson(response, AuthorizationCodeNotValid())
